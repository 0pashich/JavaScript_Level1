...
function handleClick(clickedButtonEvent) {
    ...
    // const cardNode = ;
    let product = clickedButtonEvent.target.parentNode;
    // 3. создайте литерал объекта со следующими свойствами:
    const card = {
        wrap: product, // здесь элемент с классом .product
        img: product.querySelector('img'), // здесь картинка внутри .product
        productName: product.querySelector('.productName'), // здесь .productName, который внутри .product
        button: product.querySelector('button'), // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    //   let buttonText = card.button.text;
...
function hideMoreText(card) {
    ...
    card.wrap.querySelector('.desc').outerHTML = '';
